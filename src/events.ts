...
import { DROP_TARGET_CLASS_BOTTOM, PAGEBREAK_CELL_TAG, PAGEBREAK_HEADER_TAG } from './constants';
...
function handleDragOver(event: Drag.Event, notebook: Notebook): void {
    ...
    if (index === -1) {
        ...
        notebook.widgets[notebook.widgets.length - 1].node.classList.remove(DROP_TARGET_CLASS_BOTTOM);
    }
    const targetWidget = notebook.widgets[index];
    const sourceElements = notebook.node.getElementsByClassName(DROP_SOURCE_CLASS);

    if (isValidDrop(targetWidget, sourceElements)) {
...
function handleDrop(event: Drag.Event, notebook: Notebook): void {
    ...
    notebook.widgets.forEach(cell => cell.node.classList.remove(DROP_TARGET_CLASS_BOTTOM))
    //PAGEBREAK CHANGE: check if the source and target are valid
